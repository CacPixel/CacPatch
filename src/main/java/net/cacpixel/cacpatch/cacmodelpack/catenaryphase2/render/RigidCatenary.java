...
public class RigidCatenary extends TileEntityPartsRenderer<ModelSetRail> {
	...
	public static void scriptRenderRailStatic(RailPartsRenderer renderer, TileEntityLargeRailCore tileEntity, double posX, double posY, double posZ, Parts... pArray) {
		...
		Parts overheadGWConL = pArray[14];
		Parts overheadGWConR = pArray[15];
		...
		byte overheadGroundWireType1 = getGroundWireTypeStyle1(subRails);
        ...
        */
		...
		switch (overheadGroundWireType1) {
			case 1: {
				...
				double[] p1 = rm.getRailPos(max, index);
				...
				float yaw = rm.getRailYaw(max, index);
				float railPitch = rm.getRailPitch(max, index);
				float railCant = rm.getRailRoll(max, index);


				Vec3 pos1 = new Vec3(0.6, cwHeight + 0.412, 0.0); // 模型内测量的架空地线位置（起始）
				Vec3 pos2 = new Vec3(x0 + 0.6 * NGTMath.cos(yaw), y0 + cwHeight + 0.412, z0 - 0.6 * NGTMath.sin(yaw));

				Vec3 vec = pos2.sub(pos1);
				...
				double x1 = vecX / length;
				double z1 = vecZ / length;
				double split = 16.0;
				...
				for (int j = 0; j <= split; ++j) {
					double ft = j / split;
					double f2 = (j - 8) / split;
					double fh = (f2 * f2 - 0.25) * 1;
					tessellator.addVertex((float) (vecX * ft - 0.015 * z1), (float) (vecY * ft + fh), (float) (vecZ * ft + 0.015 * x1));
					tessellator.addVertex((float) (vecX * ft + 0.015 * z1), (float) (vecY * ft + fh), (float) (vecZ * ft - 0.015 * x1));
				}
				tessellator.draw();
				...
				for (int j = 0; j <= split; ++j) {
					...
					double f2 = (j - 8) / split;
					double fh = (f2 * f2 - 0.25) * 1;
					tessellator.addVertex((float) (vecX * ft), (float) (vecY * ft + fh + 0.015), (float) (vecZ * ft));
					tessellator.addVertex((float) (vecX * ft), (float) (vecY * ft + fh - 0.015), (float) (vecZ * ft));
				}
				tessellator.draw();
	...
	private static boolean getMidPointAnchor(List<ResourceStateRail> subRails) {
		int k = subRails.size() - 1;
		for (; k >= 0; k--) {
			if (subRails.get(k).getResourceSet().getConfig().getName().contains("DC1500V_MidpointAnchorRigid")) {
				return true;
			}
		}
		return false;
	...
	private static byte getGroundWireTypeStyle1(List<ResourceStateRail> subRails) {
		int k = subRails.size() - 1;
		for (; k >= 0; k--) {
			String str = subRails.get(k).getResourceSet().getConfig().getName();
			if (str.contains("DC1500V_OverheadGroundWireRigid_style1")) {
	...
	private static byte getGroundWireTypeStyle2(List<ResourceStateRail> subRails) {
		int k = subRails.size() - 1;
		for (; k >= 0; k--) {
			String str = subRails.get(k).getResourceSet().getConfig().getName();
			if (str.contains("DC1500V_OverheadGroundWireRigid_style2")) {
				if (str.contains("Left"))
					return 1;
				else
					return 2;
